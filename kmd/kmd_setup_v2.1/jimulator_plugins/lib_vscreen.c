...
#define BASE_ADDR (0xac << 24)
/*----------------------------------------------------------------------------*/

boolean mem_r_handler (unsigned int address, unsigned int *data, int size,
                       boolean sign, boolean T, int source, boolean* abort)
{
  if ((address & (0xff << 24)) == BASE_ADDR) {
    //fprintf(stderr, "lib_vscreen: read %08x (size %d)\n", address, size);
    unsigned int offset = address - BASE_ADDR;
    *abort = FALSE;
    if (offset == vscreen_size) {  // leds
      if (size != 1)
        *abort = TRUE;
      else 
        *data = *p_LEDs;
    } else {  // framebuffer
      if (offset+size > vscreen_size) {
        *abort = TRUE;
      } else {
        switch(size) {
          case 1:
            *data = vscreen_base[offset];
            break;
          case 2:
            *data = vscreen_base[offset] << 8 + vscreen_base[offset+1];
            break;
          case 4:
            *data = vscreen_base[offset] << 24 +
                vscreen_base[offset+1] << 16 +
                vscreen_base[offset+2] << 8 +
                vscreen_base[offset+3];
            break;
          default:
            *abort = TRUE;
        }
...
{
  if ((address & (0xff << 24)) == BASE_ADDR) {
    //fprintf(stderr, "lib_vscreen: write %08x = %08x (size %d)\n", address, data, size);
    unsigned int offset = address - BASE_ADDR;
    *abort = FALSE;
    if (offset == vscreen_size) {  // leds
      if (size != 1)
      ...
      *p_handshake |= HNDSHK_LED;
      ...
      if (offset+size > vscreen_size) {
        *abort = TRUE;
        ...
        switch(size) {
          case 1:
            vscreen_base[offset] = data;
            break;
          case 2:
            vscreen_base[offset] = 0xff & (data >> 8);
            vscreen_base[offset+1] = 0xff & data;
            break;
          case 4:
            vscreen_base[offset] = 0xff & (data >> 24);
            vscreen_base[offset+1] = 0xff & (data >> 16);
            vscreen_base[offset+2] = 0xff & (data >> 8);
            vscreen_base[offset+3] = 0xff & data;
            break;
          default:
            *abort = TRUE;
        	return TRUE;
